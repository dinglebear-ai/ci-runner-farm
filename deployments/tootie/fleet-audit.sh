...
GOTIFY_URL="${GOTIFY_URL:-https://gotify.tootie.tv}"
GOTIFY_TOKEN="${GOTIFY_TOKEN:-}"
CRF_NOTIFY_SUCCESS="${CRF_NOTIFY_SUCCESS:-true}"

# Tootie's production contract. Operators may override any value in
...
CRF_EXPECTED_KACHE_ENDPOINT="${CRF_EXPECTED_KACHE_ENDPOINT:-http://10.1.0.2:9000}"
...
CRF_RUNNER_NAME_PREFIX="${CRF_RUNNER_NAME_PREFIX:-tootie-}"
...
notify_failure() {
  ...
  notify_gotify "CI Runner Farm Audit failed" "Tootie fleet verification failed. See $log_file" 8
}
notify_success() {
  local log_file="$1"
