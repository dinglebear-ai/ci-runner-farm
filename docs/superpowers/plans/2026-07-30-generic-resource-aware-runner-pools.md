...
6. Reproducible package, code-only tootie deploy, live classic validation, and rollback.
...
- Create: `docs/deployment/generic-pools-tootie.md`
...
### Step 4: Code-only deploy to tootie

Before changing live state:

1. record tootie hostname/kernel/Docker/cgroup/cpu/memory/swap/cache facts;
...
- Extend: `docs/deployment/generic-pools-tootie.md`
...
### Step 3: Stage exact code on tootie
...
No unrelated files, secrets, live credentials, temporary probe data, package binaries, or tootie backups may be committed.
...
- tootie packaged-identity gate and live workload results
...
The PR body must summarize both milestones, compatibility-gate status, tootie tests, security boundaries, rollback proof, and exact verification commands/results.
