...
This admission policy never inflates GitHub's `X-ScaleSetMaxCapacity` to manufacture lookahead. The upstream scale-set contract defines that header as the real capacity the backend may rely on when assigning work, and message responses may truncate large backlogs. Adaptive ordering therefore applies only to `JobAvailable` candidates actually present in the current response; it is not a claim of global shortest-job-first ordering over hidden GitHub backlog. A future true fast-lane/reserved-slot policy should use GitHub's separate `/{scaleSetID}/acquirablejobs` queue-introspection endpoint, with authenticated transport support added cleanly to the scale-set client, rather than abusing the capacity header or depending on long-poll timing. Rust remains the sole resource/node placement algorithm after GitHub admission.
