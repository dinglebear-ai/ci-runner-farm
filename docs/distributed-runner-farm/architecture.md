...
This deeper queue visibility also feeds a proactive, borrowable fast lane without changing GitHub's published capacity. When a pool has at least two slots and the marginal candidate that would fill the remaining capacity is a learned, non-starved job estimated at eight minutes or longer, admission acquires the higher-ranked work and deliberately leaves exactly one slot open. The reservation lasts 20 seconds. While it is active, the pool's existing supervisor heartbeat (bounded to at most ten seconds) probes authenticated `acquirablejobs` and skips the GitHub message long-poll while only learned-long work remains. Unknown work, work estimated below eight minutes, or any job protected by the ten-minute starvation override releases the hold so the next normal replay-fenced message transaction can acquire it. Expiry or lookup degradation permits exactly one long-work borrow pass, recovering utilization without immediately re-arming the reservation.

Fast-lane state is optional optimization state, never acquisition authority. Active reservations are persisted atomically beside the replay journal in a private mode-0600 `fast-lanes.json` file bounded to eight entries / 16 KiB. A restart restores a live hold, converts an already-expired hold into a one-shot borrow, and cold-starts with no reservation if the file is missing or malformed. Capacity changes, empty queue evidence, quick-work consumption, and completed borrow passes clear the durable reservation. Persistence failure keeps the live in-memory policy and logs degradation rather than failing job admission. `AcquireJobs` is never issued by the heartbeat probe or outside the existing message journal transaction, so the optimization does not create a second acquisition/replay protocol. Rust remains the sole resource/node placement algorithm after GitHub admission.
