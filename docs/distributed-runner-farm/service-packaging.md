...
The node unit intentionally uses `KillMode=process`. Restarting the node agent must not blindly terminate an adapter-managed container because that runtime may own a live job and the new agent generation must reconcile the surviving durable placement. Containers are rediscovered through the configured local adapter and persisted immutable container ID; cancellation carries that exact ID so a reused name cannot target a newer runtime. The service also enables systemd filesystem, process, device, kernel, and privilege hardening. Native execution is rejected before launch.
...
5. Run `Install-CrfNodeService.ps1` once from an elevated shell, then use `Start-Service CiRunnerFarmNode`. Verify `Get-Service CiRunnerFarmNode`, the configured service error log, the optional launch-token-bound node status projection, and `nerdctl --namespace buildkit image inspect sha256:<digest>`. Final acceptance is a real Windows-labeled GitHub job whose runner name is ephemeral, whose machine reports Windows/x64, and whose container disappears after completion or cancellation.
