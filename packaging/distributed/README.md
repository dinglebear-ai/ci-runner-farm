...
Dirty-source bundles are for local validation only and are refused by the installer so different bytes can never reuse the same immutable release identity.
