...
locked_line="$(awk -v start="$boot_line" 'NR>start && index($0,"boot_autostart_locked") { print NR; exit }' src/usr/local/emhttp/plugins/ci-runner-farm/include/runner-farm.sh)"
[ "$reconcile_line" -gt "$boot_line" ] && [ "$reconcile_line" -lt "$locked_line" ] || crf_fail 'boot reconciliation does not precede credential early return'
