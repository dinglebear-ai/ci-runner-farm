...
grep -Fq 'stop)         with_fleet_lock wait cmd_stop' "$engine" || crf_fail 'legacy stop lost dispatch-owned lock'
grep -Fq 'restart)      with_fleet_lock wait cmd_restart' "$engine" || crf_fail 'legacy restart lost dispatch-owned lock'
