...
printf '%s' "$STATUS_RESOURCES_JSON" | php -r '$j=json_decode(stream_get_contents(STDIN),true);exit(($j["available"]??false)===true&&array_key_exists("reason",$j)&&$j["reason"]===null&&($j["cpu_milli"]["configured"]??-1)===1000&&($j["cpu_milli"]["configured_headroom"]??-1)===6000&&($j["cpu_milli"]["admissible"]??-1)===6000?0:1);'
